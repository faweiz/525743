...
static err_t ftpd_accept(void *arg, struct tcp_pcb *newpcb, err_t err)
{
  ...
  strcpy(state->path, "/");
  state->userid = -1;
  state->expected_size = 0;  /* Initialize expected size */
  state->received_size = 0;  /* Initialize received size */
...
#include "xtime_l.h"  // Add this with other includes

// Add these global variables for debugging
static XTime ftp_start_time, ftp_end_time;
static u32 ftp_total_bytes = 0;

/* Buffer for accumulating data before writing to SD card */
#define FTP_WRITE_BUFFER_SIZE (64 * 1024)  /* 32KB buffer - adjust based on your needs */
static unsigned char write_buffer[FTP_WRITE_BUFFER_SIZE];
static unsigned int write_buffer_pos = 0;

/* Data connection received data - FIXED VERSION */
static err_t ftpd_data_recv(void *arg, struct tcp_pcb *tpcb, struct pbuf *p, err_t err)
{
  struct ftpd_state *state = arg;
  struct pbuf *q;
  ...
  if (p != NULL)
  {
    if (state != NULL)
    {
      if (strcasecmp(state->cmd, "STOR") == 0 || strcasecmp(state->cmd, "APPE") == 0)
      {
        ...
        if (ftp_total_bytes == 0) {
            XTime_GetTime(&ftp_start_time);
        }

        LWIP_DEBUGF(FTPD_DEBUG, ("%s: %d bytes received\n", __FUNCTION__, p->tot_len));
        ftp_total_bytes += p->tot_len;

        /* Process each pbuf in the chain */
        for (q = p; q != NULL; q = q->next)
        {
          /* If this chunk would overflow our buffer, write the buffer first */
          if (write_buffer_pos + q->len > FTP_WRITE_BUFFER_SIZE)
          {
            /* Write the full buffer to SD card */
            fr = f_write(state->fp, write_buffer, write_buffer_pos, &bw);
            if (bw != write_buffer_pos)
            {
              LWIP_DEBUGF(FTPD_DEBUG | LWIP_DBG_LEVEL_SERIOUS,
                ("%s: f_write() failed. fr=%d, wrote=%u of %u\n",
                 __FUNCTION__, fr, bw, write_buffer_pos));
              pbuf_free(p);
          ...
          memcpy(write_buffer + write_buffer_pos, q->payload, q->len);
          write_buffer_pos += q->len;
        }
      }
    }
  ...
  {
    /* Connection closed by client - THIS is the real end of file! */
    if (state != NULL)
    {
      /* Write any data left in the buffer */
      if (write_buffer_pos > 0)
      {
        ...
        {
          LWIP_DEBUGF(FTPD_DEBUG | LWIP_DBG_LEVEL_SERIOUS,
            ("%s: f_write() failed on close. fr=%d, wrote=%u of %u\n",
             __FUNCTION__, fr, bw, write_buffer_pos));
        }

        /* Calculate and display transfer speed */
        XTime_GetTime(&ftp_end_time);
        u32 time_us = (ftp_end_time - ftp_start_time) / (COUNTS_PER_SECOND / 1000000);
        u32 speed_kbps = (ftp_total_bytes * 8) / (time_us / 1000);

        xil_printf("\r\n[FTP] Transfer complete: %u bytes in %u ms (%u KB/s)\r\n",
                  ftp_total_bytes, time_us/1000, speed_kbps);
        xil_printf("[FTP] Buffer writes: ~%d writes (32KB each)\r\n",
                  ftp_total_bytes / FTP_WRITE_BUFFER_SIZE);
      ...
      /* ADD THIS CODE HERE */
	  /* Check if this was a WAV file upload */
      if (strstr(state->filename, ".wav") != NULL) {
		  strcpy(g_current_music_file, state->filename);
		  g_new_file_available = 1;
		  xil_printf("[FTP] New music file uploaded: %s\r\n", state->filename);
	  }



      LWIP_DEBUGF(FTPD_DEBUG,
        ("FTPD data connection [%s]:%d is shutdown by the client\n",
         ipaddr_ntoa(&tpcb->remote_ip), tpcb->remote_port));
      ftpd_free_data(state, FTPD_FREEDATA_CLOSE);
      
      /* Notify command handler that data connection was closed */
    ...
    {
      LWIP_DEBUGF(FTPD_DEBUG,
        ("FTPD data connection [%s]:%d is closed by the client\n",
         ipaddr_ntoa(&tpcb->remote_ip), tpcb->remote_port));
    }
  }
  return ERR_OK;
...
static err_t ftpd_data_sent(void *arg, struct tcp_pcb *tpcb, u16_t len)
{
  ...
  if (state != NULL)
  {
    if (strcasecmp(state->cmd, "LIST") == 0 || strcasecmp(state->cmd, "NLST") == 0)
...
static int ftpd_process_data_cmd(struct ftpd_state *state)
{
  ...
  if (isport)
  {
    ...
    if (isport == 4)
    {
      ...
      {
    	if (isdigit((unsigned char)state->cmdarg[i]))
          ip[i] = state->cmdarg[i];
...
cwderr:
  ...
  else if (strcasecmp(state->cmd, "LIST") == 0 || strcasecmp(state->cmd, "NLST") == 0)
  {
    if (state->cmdstep == 0)
    {
...
retrerr2:
  ...
  else if (strcasecmp(state->cmd, "STOR") == 0 || strcasecmp(state->cmd, "APPE") == 0)
  {
    if (state->cmdstep == 0)
    {
      ...
      /* Store the FULL PATH (including volume) for later playback */
	  strcpy(state->filename, buffer);  /* Now contains "0:/song.wav" */
